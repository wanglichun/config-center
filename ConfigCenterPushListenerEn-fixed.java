...
public class ConfigCenterPushListenerEn {
    ...
    private void watchNotifications() {
        try {
            // Watch the notifications base path
            List<String> notifications = zk.getChildren(NOTIFICATION_BASE_PATH, new Watcher() {
                @Override
                public void process(WatchedEvent event) {
    ...
    private void processNotification(String notification) {
        try {
            System.out.println("[NOTIFICATION] Received config change notification: " + notification);

            // Parse notification path to get group and config key
            ...
            if (parts.length >= 2) {
                ...
                String notificationPath = NOTIFICATION_BASE_PATH + "/" + notification + "/" + INSTANCE_IP;
                ...
                if (stat != null) {
                    ...
                    reportConfigStatus(groupName, configKey, configValue, getVersionFromJson(notificationJson));
