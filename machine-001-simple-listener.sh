...
watch_notifications() {
    ...
    while true; do
        ...
        local notification_data=$(docker exec zk1 zookeeper-shell localhost:2181 get "$notification_path" 2>/dev/null)
        ...
        if [ -n "$notification_data" ] && [ "$notification_data" != "null" ]; then
            ...
            # 删除通知节点
            docker exec zk1 zookeeper-shell localhost:2181 delete "$notification_path" 2>/dev/null
        ...
        local db_notification_data=$(docker exec zk1 zookeeper-shell localhost:2181 get "$db_notification_path" 2>/dev/null)
        ...
        if [ -n "$db_notification_data" ] && [ "$db_notification_data" != "null" ]; then
            ...
            docker exec zk1 zookeeper-shell localhost:2181 delete "$db_notification_path" 2>/dev/null
        ...
        local common_notification_data=$(docker exec zk1 zookeeper-shell localhost:2181 get "$common_notification_path" 2>/dev/null)
        ...
        if [ -n "$common_notification_data" ] && [ "$common_notification_data" != "null" ]; then
            ...
            docker exec zk1 zookeeper-shell localhost:2181 delete "$common_notification_path" 2>/dev/null
