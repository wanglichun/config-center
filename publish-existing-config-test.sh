...
# 提取未发布的配置ID
UNPUBLISHED_IDS=$(echo "$CONFIG_RESPONSE" | grep -o '"id":[0-9]*,"status":"ACTIVE"' | grep -o '"id":[0-9]*' | cut -d':' -f2 | head -3)
